...
def extract_python_code(llm_output: str) -> str | None:
    ...
    if not isinstance(llm_output, str):
        logger.warning(
            f"extract_python_code received non-string input: {type(llm_output)}"
        )
        return None
    ...
    if match_generic:
        logger.warning(
            "extract_python_code: Found generic code block, assuming Python for single file extraction."
        )
        return match_generic.group(1).strip()

    logger.warning(
        "extract_python_code: No Python code block found for single file extraction."
    )
    return None
...
def extract_project_structure_from_llm(llm_output: str) -> Dict[str, Any] | None:
    ...
    if not isinstance(llm_output, str):
        logger.warning(
            f"extract_project_structure_from_llm received non-string input: {type(llm_output)}"
        )
        return None
    ...
    try:
        ...
        if (
            ...
            or "files" not in project_data
            or not isinstance(project_data["files"], list)
        ):
            logger.error(
                f"extract_project_structure_from_llm: Parsed JSON is not a valid project structure. Missing 'files' list. Parsed: {str(project_data)[:200]}"
            )
            return None
        for file_info in project_data["files"]:
            if (
                ...
                or "content" not in file_info
            ):
                logger.error(
                    f"extract_project_structure_from_llm: Invalid file entry in project structure: {file_info}"
                )
                return None
        return project_data
    except json.JSONDecodeError as e:
        logger.error(
            f"extract_project_structure_from_llm: JSONDecodeError: {e}. Content: {json_text_to_parse[:500]}..."
        )
        return None
    except Exception as e:
        logger.error(
            f"extract_project_structure_from_llm: Unexpected error during parsing: {e}. Content: {json_text_to_parse[:500]}..."
        )
        return None
...
def code_tester_tool(
    project_structure: Dict[
        str, Any
    ],  # {"files": [{"name": "file.py", "content": "..."}], "entry_point_file": "main.py"}
    test_cases: List[
        Dict[str, Any]
    ],  # Each test case: {"inputs": (args,), "expected_output": val, "description": "...", "target_function": "func_name", "target_file": "file.py" (optional)}
) -> List[Dict[str, Any]]:
    """
            ...
            results.append(
                {
                    "test_case": tc,
                    "status": "error",
                    "message": "No project structure provided.",
                    "actual_output": None,
                }
            )
        return results
    ...
    logger.info(
        f"code_tester_tool: Created temporary directory for testing: {temp_dir}"
    )
    ...
    try:
        ...
        main_function_to_test = project_structure.get(
            "main_function_to_test"
        )  # Could be used if test_cases don't specify func
        ...
        for test_case in test_cases:
            ...
            target_function_name = test_case.get(
                "target_function", main_function_to_test
            )
                ...
                results.append(
                    {
                        "test_case": test_case,
                        "status": "error",
                        "message": "Test case missing target_function name.",
                        "actual_output": None,
                    }
                )
                continue
            if not target_file_name:
                logger.error(
                    f"Test case '{description}' for function '{target_function_name}' missing target_file name."
                )
                results.append(
                    {
                        "test_case": test_case,
                        "status": "error",
                        "message": "Test case missing target_file name.",
                        "actual_output": None,
                    }
                )
                continue

            module_name = os.path.splitext(target_file_name)[0].replace(
                os.path.sep, "."
            )
            ...
            try:
                ...
                for file_info_compile in project_structure["files"]:
                    compile_file_path = os.path.join(
                        temp_dir, file_info_compile["name"]
                    )
                ...
                imported_module = __import__(
                    module_name, fromlist=[target_function_name]
                )
                if not hasattr(imported_module, target_function_name):
                    raise AttributeError(
                        f"Function '{target_function_name}' not found in module '{module_name}'."
                    )
                func_to_test = getattr(imported_module, target_function_name)

            except ImportError as e:
                logger.error(
                    f"code_tester_tool: ImportError for module '{module_name}': {e}",
                    exc_info=True,
                )
                results.append(
                    {
                        "test_case": test_case,
                        "status": "import_error",
                        "message": f"Could not import module '{module_name}': {e}",
                        "actual_output": None,
                    }
                )
                ...
                logger.error(
                    f"code_tester_tool: AttributeError for function '{target_function_name}' in '{module_name}': {e}",
                    exc_info=True,
                )
                results.append(
                    {
                        "test_case": test_case,
                        "status": "error",
                        "message": f"Function '{target_function_name}' not found in '{module_name}': {e}",
                        "actual_output": None,
                    }
                )
                continue
                ...
                Exception
            ) as e:  # Broad catch for compilation/initialization errors of the project code
                logger.error(
                    f"code_tester_tool: Error setting up test environment for '{target_file_name}': {e}",
                    exc_info=True,
                )
                results.append(
                    {
                        "test_case": test_case,
                        "status": "setup_error",
                        "message": f"Error setting up test environment: {e}",
                        "actual_output": None,
                    }
                )
                continue
            ...
            if not isinstance(inputs, tuple):
                logger.warning(
                    f"Test case '{description}' inputs not a tuple, wrapping: {inputs}"
                )
                inputs = (inputs,)
            ...
            try:
                ...
                logger.error(
                    f"code_tester_tool: Runtime error during test '{description}' for '{target_function_name}': {e}",
                    exc_info=True,
                )
                results.append(
                    {
        ...
        try:
            ...
            logger.error(
                f"code_tester_tool: Error removing temporary directory {temp_dir}: {e}"
            )
...
if __name__ == "__main__":
    ...
    """
    project_structure_raw = extract_project_structure_from_llm(
        llm_project_output_raw_json
    )
    print(
        f"Extracted Project Structure (raw json): {json.dumps(project_structure_raw, indent=2)}"
    )
    ...
    ```
    """
    project_structure_generic_md = extract_project_structure_from_llm(
        llm_project_output_md_generic
    )
    print(
        f"Extracted Project Structure (generic md): {json.dumps(project_structure_generic_md, indent=2)}"
    )
    ...
    assert (
        project_structure_invalid is None
        or project_structure_invalid.get("error") is not None
    )
    ...
    test_cases_multi = [
        {
            "inputs": (2, 3, 4),
            "expected_output": 20,
            "description": "Test main_calculation",
            "target_function": "main_calculation",
            "target_file": "main.py",
        },
        {
            "inputs": (10, 5),
            "expected_output": 15,
            "description": "Test helper_add directly",
            "target_function": "helper_add",
            "target_file": "utils.py",
        },
        {
            "inputs": (1, 1, 1),
            "expected_output": 0,
            "description": "Intentional fail main_calculation",
            "target_function": "main_calculation",
            "target_file": "main.py",
        },
    ]
    ...
    project_import_error = {
        "files": [
            {
                "name": "main.py",
                "content": "from non_existent_module import some_func\n\ndef run_it():\n  return some_func()",
            }
        ]
    }
    test_cases_import_error = [
        {
            ...
            "target_function": "run_it",
            "target_file": "main.py",
        }
    ]
    results_import_error = code_tester_tool(
        project_import_error, test_cases_import_error
    )
    print("\nTest results for import error project:")
    for res in results_import_error:
        print(res)
    assert results_import_error[0]["status"] == "import_error"

    project_no_target_file = {
        "files": [{"name": "my_code.py", "content": "def my_func(): return 10"}]
    }
    test_no_target_file = [
        {
            "inputs": (),
            "expected_output": 10,
            "description": "No target file in test",
            "target_function": "my_func",
        }
    ]
    results_no_target_file = code_tester_tool(
        project_no_target_file, test_no_target_file
    )
    print(
        "\nTest results for project with no target_file in test_case (should fail to find func):"
    )
    for res in results_no_target_file:
        print(res)
    assert (
        results_no_target_file[0]["status"] == "error"
    )  # or "setup_error" depending on how it's caught
