...
def create_task(initial_request: str, db_file: str = DB_FILE) -> Optional[int]:
    ...
    return execute_query(db_file, query, (initial_request, 'created', now, now), last_row_id=True)
        ...
        for field in ["architectural_decision", "planned_task_description", "generated_test_cases_json"]: 
            if field in task_data and task_data[field]:
                try:
                    ...
                    logger.warning(f"Could not parse JSON for field {field} in task {task_id}. Value: {task_data[field]}. Error: {e}")
...
def update_task_field(task_id: int, field_name: str, field_value: Any, db_file: str = DB_FILE) -> bool:
    ...
    return execute_query(db_file, query, (value_to_store, datetime.now().isoformat(), task_id)) is True

def add_code_version(task_id: int, code_project_structure_json: str, iteration: int, db_file: str = DB_FILE) -> Optional[int]:
    # ... (same as before)
    query = "INSERT INTO code_versions (task_id, generated_code, iteration_number) VALUES (?, ?, ?)"
    return execute_query(db_file, query, (task_id, code_project_structure_json, iteration), last_row_id=True)
    ...
    if row: 
        ...
        if data.get("generated_code"):
            try:
                ...
                logger.warning(f"Could not parse JSON for generated_code in version {data['code_version_id']}. Error: {e}")
                data["generated_code"] = {"error": "Failed to parse code structure", "raw": data["generated_code"]}
        return data
    return None
...
def get_validation_log_for_version(code_version_id: int, db_file: str = DB_FILE) -> Optional[Dict[str, Any]]:
    ...
    if row:
        ...
        if data.get("issues_found"): 
            try: 
                ...
                data["issues_found"] = parsed_issues if isinstance(parsed_issues, list) else [str(parsed_issues)]
...
def get_packaged_artifact(task_id: int, db_file: str = DB_FILE) -> Optional[Dict[str, Any]]:
    ...
    if row:
        ...
        if artifact_data["info"]:
            try: artifact_data["info"] = json.loads(artifact_data["info"])
            except (json.JSONDecodeError, TypeError): artifact_data["info"] = {"error":"parsing failed", "raw":artifact_data["info"]}
...
def load_task_state_from_db(task_id: int, db_file: str = DB_FILE) -> Optional[Dict[str, Any]]:
    ...
    loaded_state = {
        ...
        "suggested_project_outline": task_data.get("planned_task_description", {}).get("suggested_project_structure") if isinstance(task_data.get("planned_task_description"),dict) else None,
    ...
    if loaded_state["generated_test_cases"] and isinstance(loaded_state["generated_test_cases"], list):
        for tc in loaded_state["generated_test_cases"]:
            if isinstance(tc, dict) and "inputs" in tc and isinstance(tc["inputs"], list):
                tc["inputs"] = tuple(tc["inputs"])
            elif isinstance(tc, dict) and "inputs" in tc and not isinstance(tc["inputs"], tuple): # Handle single non-list item
                tc["inputs"] = (tc["inputs"],)


    latest_code_version = get_latest_code_version(task_id, db_file) 
    if latest_code_version:
        loaded_state["generated_project_structure"] = latest_code_version.get("generated_code") 
        loaded_state["active_code_version_id"] = latest_code_version.get("code_version_id")
        if loaded_state["active_code_version_id"]:
            loaded_state["test_results_summary"] = get_test_results_for_version(loaded_state["active_code_version_id"], db_file)
            val_log = get_validation_log_for_version(loaded_state["active_code_version_id"], db_file) 
            if val_log:
            ...
            if loaded_state["test_results_summary"]:
                loaded_state["all_tests_passed"] = all(tr.get("status") == "success" for tr in loaded_state["test_results_summary"])
    ...
    logger.info(f"Successfully loaded state for task {task_id}. Resuming at stage: {loaded_state['ui_stage']}")
    return loaded_state

if __name__ == "__main__":
    db_dir_main = os.path.dirname(TEST_DB_FILE) 
    if db_dir_main and not os.path.exists(db_dir_main): os.makedirs(db_dir_main)
    
    current_db_to_use = str(TEST_DB_FILE) # Use the test DB for this main block too
    if os.path.exists(current_db_to_use): os.remove(current_db_to_use)
    ...
    if task_id1:
        ...
        update_task_field(task_id1, "status", "planning_done", db_file=current_db_to_use)
        data1_updated = get_task_data(task_id1, db_file=current_db_to_use)
        print(f"Updated task 1 status: {data1_updated['status']}, updated_at: {data1_updated['updated_at']}")

    print("\n--- Testing Code Versioning ---")
    if task_id1:
        code_v1 = {"files": [{"name": "hello.py", "content": "def hello(): print('world')"}]}
        cv1_id = add_code_version(task_id1, json.dumps(code_v1), 0, db_file=current_db_to_use)
    ...
    task_id2 = create_task("Create a complex calculation utility.", db_file=current_db_to_use)
    if task_id1 and task_id2:
        ...
        if loaded_state1:
            print(f"Loaded state for task {task_id1} (status: {loaded_state1['status']}):")
        else:
