...
def call_llm(messages: list, model: str = DEFAULT_MODEL, temperature: float = 0.2, expect_json: bool = True) -> str:
    ...
    try:
        ...
        if content is None:
            ...
            return json.dumps({"error": "LLM returned no content"}) # Ensure valid JSON string
        return content
...
if __name__ == "__main__":
    ...
    test_messages_json = [
        {"role": "system", "content": "You are a helpful assistant."}, # Removed "Respond in JSON" here
        {"role": "user", "content": "What is the meaning of life? Respond with a key 'answer'. Please ensure your entire response is a single, valid JSON object."} # Added JSON instruction here
    ]
    if os.getenv("OPENAI_API_KEY"):
        ...
        try:
            ...
            if "error" in response_json_parsed: print("LLM call resulted in an error.")
        except json.JSONDecodeError as e: print(f"Failed to parse LLM JSON response: {e}")
        ...
        if response_str_code.startswith('{"error"'): # Check if it's our JSON error format
            try:
                parsed_error = json.loads(response_str_code)
                if "error" in parsed_error:
                     print(f"LLM call for code resulted in an error: {parsed_error['error']}")
            except:
