...
class TestDatabaseFunctions(unittest.TestCase):
    ...
    def test_03_update_task_field_with_json_and_plain_text(self):
        ...
        self.assertIsNotNone(task_id)
        update_success_status = database.update_task_field(
            task_id, "status", "planning"
        )
        ...
        current_dt = datetime.fromisoformat(
            original_updated_at
        )  # Make sure original_updated_at is valid ISO
        ...
        with patch("utils.database.datetime") as mock_datetime_module:
            ...
            update_success_json = database.update_task_field(
                task_id, "architectural_decision", arch_decision_dict
            )
        ...
        test_cases_list = [
            {"inputs": [1, 2], "expected_output": 3, "description": "add test"}
        ]
        update_success_tests = database.update_task_field(
            task_id, "generated_test_cases_json", test_cases_list
        )
    ...
    def test_05_test_run_results(self):
        ...
        log_id = database.log_test_run_result(
            task_id,
            cv_id,
            "Test case 1 desc",
            "success",
            {"output": 5},
            "Passed: Test case 1 desc",
        )
        ...
        self.assertEqual(
            results[0]["test_case"]["description"], "Test case 1 desc"
        )  # Access description from nested test_case
    ...
    def test_06_validation_logs(self):
        ...
        cv_id = database.add_code_version(task_id, json.dumps({"files": []}), 0)
        log_id = database.log_validation_result(
            task_id, cv_id, "fail", ["Issue 1", "Issue 2"]
        )
        self.assertIsNotNone(log_id)
    ...
    def test_07_feedback_logs(self):
        ...
        cv_id = database.add_code_version(task_id, json.dumps({"files": []}), 0)
        log_id = database.log_feedback(
            task_id, cv_id, "user_rejection", "Needs multi-file support."
        )
        self.assertIsNotNone(log_id)
    ...
    def test_08_packaged_artifacts(self):
        ...
        info = {
            "project_files": [{"name": "final.py", "content": "final code"}],
            "entry_point": "final.py",
        }
    ...
    def test_09_get_all_tasks_summary(self):
        ...
        summary_for_task1 = next(
            (s for s in summaries if s["task_id"] == task_id1), None
        )
    ...
    def test_10_load_task_state_from_db(self):
        ...
        self.assertIsNotNone(task_id)
        arch_dec = {"lang": "python"}
        plan_desc = {
            "name": "func_a",
            "target_file": "main.py",
            "component_name": "func_a",
        }  # ensure component_name
        test_cases_json_list = [
            {
                "inputs": (1,),
                "expected_output": 2,
                "description": "test1",
        ...
        database.update_task_field(
            task_id, "generated_test_cases_json", test_cases_json_list
        )
        database.update_task_field(task_id, "status", "human_review")
        database.update_task_field(task_id, "refinement_count", 1)

        cv1_code = {
            "files": [{"name": "main.py", "content": "def func_a(x): return x"}]
        }
        cv1_id = database.add_code_version(task_id, json.dumps(cv1_code), 0)
        cv2_code = {
            "files": [{"name": "main.py", "content": "def func_a(x): return x + 1"}]
        }
        cv2_id = database.add_code_version(task_id, json.dumps(cv2_code), 1)
        database.log_test_run_result(
            task_id, cv2_id, "test1 desc", "success", 2, "Passed for input (1)"
        )
        database.log_validation_result(task_id, cv2_id, "pass", [])
        database.log_feedback(
            task_id, cv1_id, "user_rejection", "Initial code was wrong."
        )
