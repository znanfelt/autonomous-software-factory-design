# tests/test_nodes.py
import unittest
from unittest.mock import patch, MagicMock # Ensure MagicMock is imported if used, though not in current snippets
import sys
import os
...
class TestArchitectPlannerNode(unittest.TestCase):
    ...
    def test_exec_plan_ready_first_try(self, mock_call_llm):
        mock_call_llm.side_effect = [
            ...
            json.dumps({"planned_task_description": {"component_name": "add", "target_file": "main.py"}, "planner_notes": "Be polite.", "clarification_questions": []})
        ]
...
class TestDeveloperNode(unittest.TestCase): # Assuming previous DeveloperNode tests were fine
    def setUp(self):
        self.node = DeveloperNode()
        self.shared_state = {
            "planned_task_description": {"component_name": "add", "target_file": "main.py"},
            "suggested_project_outline": [{"file_name":"main.py", "purpose":"main logic"}],
    ...
    def test_exec_success_code_extraction(self, mock_call_llm):
        mock_call_llm.return_value = json.dumps({"files": [{"name":"main.py", "content": "def add(a,b): return a+b"}]})
...
class TestTestCaseDesignerNode(unittest.TestCase): # Assuming previous tests were fine
    ...
    def test_exec_success(self, mock_call_llm):
        ...
        mock_call_llm.return_value = json.dumps({
            "test_cases": [{"target_file": "main.py", "target_function": "add", "inputs": (1,2), "expected_output": 3, "description": "Add."}]
        })
        prep = node.prep({
            "planned_task_description": {"component_name": "add", "target_file":"main.py"},
            "planner_notes": "", "llm_models_config": {},
            "current_project_structure_json_str": "{}", "critique_feedback_for_tests": "N/A"
        })
...
class TestValidationNode(unittest.TestCase): # Assuming previous tests were fine
    ...
    def test_exec_success(self, mock_call_llm):
        ...
        prep = node.prep({
            ...
            "planner_notes": "notes", "validation_rules_context": "rules", "llm_models_config": {}
        })
        result = node.exec(prep)
...
class TestCritiqueNode(unittest.TestCase): # Assuming previous tests were fine
    ...
    def test_exec_success(self, mock_call_llm):
        ...
        prep = node.prep({
            ...
            "planner_notes": "notes", "test_failure_message": "fail",
            "validation_issues": [], "user_rejection_reason": "none",
            "debugging_tips_context": "tips", "llm_models_config": {}
        })
        result = node.exec(prep)
...
class TestPackageNode(unittest.TestCase):
    def test_exec_success(self):
        ...
        self.assertEqual(result["packaged_artifacts_info"]["project_files"][0]["name"], "main.py")
        self.assertEqual(result["packaged_artifacts_info"]["project_files"][0]["content"], "def foo(): pass")
    ...
    def test_exec_missing_project_structure(self):
        ...
        self.assertEqual(result["error"], "Generated project structure missing or invalid for packaging.")
    ...
    def test_exec_missing_component_name_in_plan(self):
        ...
        result = node.exec(prep_data_for_exec) # prep_data is valid, exec will find missing component_name
        self.assertIn("error", result)
        # This error is now generated inside exec if component_name is missing
        self.assertEqual(result["error"], "component_name missing in planned_task_description for packaging.")
