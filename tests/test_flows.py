...
class TestElicitationFlow(unittest.TestCase):
    ...
    def test_elicitation_needs_clarification(self, mock_call_llm):
        ...
        mock_call_llm.side_effect = [
            ...
            json.dumps(
                {"error": "Cannot make plan, ambiguous"}
            ),  # Simulate planner failing to make direct plan
...
class TestTestDesignFlow(unittest.TestCase):
    ...
    def test_test_design_flow_success(self, mock_call_llm):
        ...
        self.assertEqual(
            shared_state["generated_test_cases"][0]["target_function"], "add"
        )
...
class TestInitialCodeGenFlow(unittest.TestCase):
    ...
    def test_initial_code_gen_flow_success(self, mock_call_llm):
        mock_call_llm.return_value = json.dumps(
            {
                "files": [
                    {"name": "main.py", "content": "def add(a, b):\n  return a + b"}
                ],
                "entry_point_file": "main.py",
                "main_function_to_test": "add",
            }
        )
        shared_state = {
            "planned_task_description": {
            ...
            "developer_task_description": json.dumps(
                {"component_name": "add", "target_file": "main.py"}
        ...
        self.assertEqual(
            shared_state["generated_project_structure"]["files"][0]["name"], "main.py"
        )
...
class TestQAValidationFlow(unittest.TestCase):
    ...
    def test_qa_validation_security_flow_all_pass(
        self, mock_call_llm_validation, mock_code_tester_tool
    ):
        ...
        mock_call_llm_validation.return_value = json.dumps(
        ...
        action = qa_validation_security_flow.run(
            shared_state
        )  # Last node is ValidationNode, post() returns None ("default")
    ...
    @patch("nodes.code_tester_tool")
    @patch("nodes.call_llm")
    def test_qa_validation_security_flow_test_fail(
        self, mock_call_llm_validation, mock_code_tester_tool
    ):
        mock_code_tester_tool.return_value = [
            {
        ...
        shared_state = {
            "generated_project_structure": {
                "files": [{"name": "main.py", "content": "def foo(): return 0"}]
            },
            "generated_test_cases": [
                {
                    "target_file": "main.py",
...
class TestCritiqueRefineFlow(unittest.TestCase):
    @patch(
        "nodes.call_llm"
    )  # This will mock call_llm for both CritiqueNode and DeveloperNode
    def test_critique_refine_flow_success(self, mock_call_llm):
        ...
        shared_state = {
            "planned_task_description": {
                "component_name": "my_func",
                "target_file": "main.py",
            },
            "developer_task_description": json.dumps(
                {"component_name": "my_func", "target_file": "main.py"}
            ),  # Stringified plan
            "generated_project_structure": {
                "files": [{"name": "main.py", "content": "def my_func(): return False"}]
            },
        ...
        self.assertEqual(
            shared_state["generated_project_structure"]["files"][0]["name"],
            "main_v2.py",
        )
...
class TestPackagingFlow(unittest.TestCase):
    def test_packaging_flow_success(self):
        ...
        shared_state = {
            "generated_project_structure": {
                "files": [{"name": "final_app.py", "content": "print('done')"}],
                "entry_point_file": "final_app.py",
            },
            "planned_task_description": {
                "component_name": "final_app",
                "target_file": "final_app.py",
            },
        }
