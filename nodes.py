...
from utils.prompts import (
    ...
    CRITIQUE_PROMPT_TEMPLATE, SECURITY_COMPLIANCE_PROMPT_TEMPLATE # New
)
...
class SimpleJsonOutputParser: # Same as before
    ...
    def parse(self, text: str) -> Any:
        try:
            ...
            if match_md_json: json_text_to_parse = match_md_json.group(1).strip()
            elif match_md_generic: json_text_to_parse = match_md_generic.group(1).strip()
            else:
                match_obj = re.search(r"^\s*\{.*\}\s*$", text, re.DOTALL)
                if match_obj: json_text_to_parse = match_obj.group(0).strip()
                else: json_text_to_parse = text.strip()
            if not json_text_to_parse: return {"error": "JSON parsing failed: No JSON content identified", "raw_text": text}
            return json.loads(json_text_to_parse)
        except json.JSONDecodeError as e:
            raw_snippet = json_text_to_parse[:200] if json_text_to_parse else text[:200]
            return {"error": f"JSON parsing failed: {e}", "raw_json_text": json_text_to_parse, "original_text": text, "snippet": raw_snippet}
        except Exception as e: return {"error": f"Unexpected parsing error: {e}", "raw_text": text}
...
class InitialRequestNode(Node): # Same as before
    ...
    def exec(self, prep_res: Optional[str]) -> Optional[str]:
        ...
        if not prep_res: logger.error("InitialRequestNode: No user request provided."); return None
        return prep_res
    def post(self, shared: Dict[str, Any], prep_res: Optional[str], exec_res: Optional[str]):
        logger.info("InitialRequestNode - Post")
        if exec_res is None: shared["current_error_message"] = "Initial request was empty."; return "error_encountered" 
        shared["initial_user_request"] = exec_res; shared["current_request_for_planner"] = exec_res 
        logger.debug(f"Initial request stored: {exec_res[:100]}..."); return "default"

class ArchitectPlannerNode(Node): # Same as before
    # ...
    def prep(self, shared: Dict[str, Any]) -> Dict[str, Any]:
        logger.info(f"ArchitectPlannerNode - Prep. ID: {id(shared)}. planner_iter: {shared.get('planner_iteration_count')}")
        return {"current_request": shared.get("current_request_for_planner", ""),"architectural_principles_context": shared.get("architectural_principles_context", "N/A"), "planning_guidelines_context": shared.get("planning_guidelines_context", "N/A"), "llm_models_config": shared.get("llm_models_config", {})}
    def exec(self, prep_res: Dict[str, Any]) -> Dict[str, Any]:
        logger.info(f"ArchitectPlannerNode - Exec with request: {prep_res['current_request'][:100]}...")
        current_request = prep_res["current_request"]; arch_principles_ctx = prep_res["architectural_principles_context"]; plan_guidelines_ctx = prep_res["planning_guidelines_context"]; llm_models_config = prep_res["llm_models_config"]
        ...
        if clar_output.get("error"): logger.error(f"Planner Clarification LLM error: {clar_output.get('error')}"); return {"error": "Planner Clarification LLM failed", "details": clar_output.get("raw_text", clar_response_str), "architect_decision": arch_decision}
        logger.info(f"Planner generated clarification questions: {clar_output.get('clarification_questions')}"); return {"architect_decision": arch_decision, "planned_output": clar_output, "needs_clarification": True}
    def post(self, shared: Dict[str, Any], prep_res: Dict[str, Any], exec_res: Dict[str, Any]):
        logger.info(f"ArchitectPlannerNode - Post. ID shared: {id(shared)}. planner_iter entry: {shared.get('planner_iteration_count')}")
        shared["planner_iteration_count"] = shared.get("planner_iteration_count", -1) + 1 # Ensure it starts at 0 then increments
        logger.info(f"ArchitectPlannerNode: planner_iteration_count in shared is now: {shared['planner_iteration_count']}")
        if exec_res.get("error"): shared["current_error_message"] = f"{exec_res['error']}: {str(exec_res.get('details', ''))[:200]}"; logger.error(f"APNode error: {shared['current_error_message']}"); return "error_encountered" 
        shared["architectural_decision"] = exec_res.get("architect_decision"); planned_output = exec_res.get("planned_output", {})
        if exec_res.get("needs_clarification") and planned_output.get("clarification_questions"):
            shared["clarification_questions_for_user"]=planned_output["clarification_questions"]; shared["planned_task_description"]=None; shared["planner_notes"]=None; shared["suggested_project_outline"]=None; return "clarification_needed"
        elif planned_output.get("planned_task_description"):
            shared["planned_task_description"]=planned_output["planned_task_description"]; shared["planner_notes"]=planned_output.get("planner_notes"); shared["suggested_project_outline"]=planned_output.get("suggested_project_structure")
            shared["developer_task_description"]=json.dumps(planned_output["planned_task_description"],indent=2); shared["clarification_questions_for_user"]=None; return "plan_ready_for_code"
        else: shared["current_error_message"]="Planner no plan/clarification."; logger.error(f"APNode error: {shared['current_error_message']} - Output: {planned_output}"); return "error_encountered"

class DeveloperNode(Node): # Same as before (with logging fix)
    # ...
    def prep(self, shared: Dict[str, Any]) -> Dict[str, Any]:
        logger.info(f"DeveloperNode - Prep. ID shared: {id(shared)}. refinement_count: {shared.get('refinement_count')}")
        planned_task_desc_obj=shared.get("planned_task_description"); suggested_project_outline_obj=shared.get("suggested_project_outline") 
        if not isinstance(planned_task_desc_obj,dict): return {"error": "Plan (obj) missing."}
        if suggested_project_outline_obj and not isinstance(suggested_project_outline_obj,list): suggested_project_outline_obj=[]
        return {"planned_task_description_json_str":json.dumps(planned_task_desc_obj,indent=2),"suggested_project_outline_json_str":json.dumps(suggested_project_outline_obj or [],indent=2),"planner_notes":shared.get("planner_notes","N/A"),"coding_standards_context":shared.get("coding_standards_context","N/A"),"critique_feedback":shared.get("critique_feedback","N/A"),"full_feedback_history":"\n".join([f"- {i}" for i in shared.get("feedback_history",[])])or "No prior feedback.","llm_models_config":shared.get("llm_models_config",{})}
    def exec(self, prep_res: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        if prep_res.get("error"): logger.error(f"DevNode: Prep error: {prep_res['error']}"); return prep_res
        logger.info(f"DevNode - Exec plan: {prep_res['planned_task_description_json_str'][:100]}...")
    ...
    def post(self, shared: Dict[str, Any], prep_res: Dict[str, Any], exec_res: Optional[Dict[str, Any]]):
        logger.info(f"DeveloperNode - Post. ID shared: {id(shared)}. Refinement_count entry: {shared.get('refinement_count')}")
        shared["refinement_count"]=shared.get("refinement_count",-1)+1; current_attempt_number=shared["refinement_count"]
        logger.info(f"DeveloperNode: Post-increment. refinement_count in shared: {shared['refinement_count']}")
        if exec_res is None or exec_res.get("error"):
            error_detail=exec_res.get("details",exec_res.get("raw_llm_response","Unknown dev error.")) if isinstance(exec_res,dict) else "Exec returned None"
            shared["current_error_message"]=f"Dev Error (Att.{current_attempt_number}): {exec_res.get('error','Code gen failed')}. Details: {str(error_detail)[:100]}"
...
class QANode(Node): # Same as before
    ...
    def prep(self, shared: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        project_structure = shared.get("generated_project_structure"); test_cases = shared.get("generated_test_cases"); current_idx = shared.get("current_test_case_index", 0)
    ...
    def exec(self, prep_res: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        if not prep_res or prep_res.get("error"): error_msg = prep_res.get("error") if prep_res else "Prep failed"; return {"status": "error", "message": error_msg, "test_case": prep_res.get("test_case") if prep_res else None}
        project_structure = prep_res["project_structure"]; test_case = prep_res["test_case"]
...
class ValidationNode(Node): # Same as before
    ...
    def prep(self, shared: Dict[str, Any]) -> Dict[str, Any]:
        ...
        planned_task_desc_obj=shared.get("planned_task_description"); project_structure_obj=shared.get("generated_project_structure")
        if not isinstance(planned_task_desc_obj,dict): return {"error":"Plan missing."}
        if not isinstance(project_structure_obj,dict) or not project_structure_obj.get("files"): return {"error":"Project structure missing."}
        return {"project_structure_json_str":json.dumps(project_structure_obj,indent=2),"task_description_json_str":json.dumps(planned_task_desc_obj,indent=2),"planner_notes":shared.get("planner_notes","N/A"),"validation_rules_context":shared.get("validation_rules_context","N/A"),"llm_models_config":shared.get("llm_models_config",{})}
    def exec(self, prep_res: Dict[str, Any]) -> Dict[str, Any]:
        if prep_res.get("error"): return {"validation_passed":False,"issues_found":[prep_res['error']]}
        logger.info(f"ValidationNode - Exec plan: {prep_res['task_description_json_str'][:100]}...")
        llm_model=prep_res["llm_models_config"].get("validation_llm","gpt-4o"); val_prompt=VALIDATION_PROMPT_TEMPLATE.format(**prep_res)
        ...
        if validation_result.get("error"): return {"validation_passed":False,"issues_found":[f"Validation LLM fail: {validation_result['error']}"],"details":validation_result.get("raw_text",response_str)}
...
class SecurityComplianceNode(Node):
    def prep(self, shared: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(planned_task_desc_obj, dict):
            return {"error": "Planned task description (object) missing for security check."}
        if not isinstance(project_structure_obj, dict) or not project_structure_obj.get("files"):
            return {"error": "Generated project structure missing or invalid for security check."}
        ...
        return {
            ...
            "security_compliance_rules_context": shared.get("security_compliance_rules_context", "N/A - No specific rules provided."),
            "llm_models_config": shared.get("llm_models_config", {})
        }

    def exec(self, prep_res: Dict[str, Any]) -> Dict[str, Any]:
        if prep_res.get("error"):
            logger.error(f"SecurityComplianceNode: Error from prep: {prep_res['error']}")
            return {"security_compliance_passed": False, "issues_identified": [prep_res['error']]}

        logger.info(f"SecurityComplianceNode - Executing for plan: {prep_res['task_description_json_str'][:100]}...")
        llm_model = prep_res["llm_models_config"].get("validation_llm", "gpt-4o") # Reuse validation model or define a new one
        ...
        response_str = call_llm(messages=[{"role": "user", "content": sec_prompt}], model=llm_model, temperature=0.1, expect_json=True)
        security_result = SimpleJsonOutputParser().parse(response_str)

        if security_result.get("error"):
            logger.error(f"SecurityCompliance LLM error or parsing failed: {security_result['error']}")
            return {"security_compliance_passed": False, "issues_identified": [f"Security LLM failed: {security_result['error']}"], "details": security_result.get("raw_text", response_str)}
    ...
    def post(self, shared: Dict[str, Any], prep_res: Dict[str, Any], exec_res: Dict[str, Any]):
        ...
        if isinstance(exec_res, dict) and "security_compliance_passed" in exec_res:
            shared["security_compliance_status"] = "pass" if exec_res["security_compliance_passed"] and not exec_res.get("issues_identified") else "fail"
            shared["security_compliance_issues"] = exec_res.get("issues_identified", [])
            if not isinstance(shared["security_compliance_issues"], list):
                shared["security_compliance_issues"] = [str(shared["security_compliance_issues"])] if shared["security_compliance_issues"] else []
            if exec_res.get("security_compliance_passed") and shared["security_compliance_issues"]:
                 shared["security_compliance_status"] = "fail"; shared["security_compliance_issues"].append("Internal Consistency: LLM reported security pass but listed issues.")
        else:
            shared["security_compliance_status"] = "error"
            shared["security_compliance_issues"] = [str(exec_res.get("details", "Security agent returned malformed output."))]

        logger.debug(f"Security check status: {shared['security_compliance_status']}, Issues: {shared['security_compliance_issues']}")
        if shared["security_compliance_status"] != "pass" and shared["security_compliance_issues"]:
            shared.setdefault("feedback_history",[]).append(f"Security/Compliance Issues (DevAttempt {shared.get('refinement_count',0)}): {'; '.join(shared['security_compliance_issues'])}")
...
class CritiqueNode(Node): # Same as before
    ...
    def prep(self, shared: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"task_description_json_str":json.dumps(planned_task_desc_obj,indent=2),"planner_notes":shared.get("planner_notes","N/A"),"project_structure_json_str":json.dumps(project_structure_obj,indent=2),"test_failure_message":shared.get("current_test_message","N/A"),"validation_issues_list_str":"; ".join(shared.get("validation_issues",[]))if shared.get("validation_issues")else "N/A","user_rejection_reason":shared.get("user_rejection_reason","N/A"),"debugging_tips_context":shared.get("debugging_tips_context","N/A"),"llm_models_config":shared.get("llm_models_config",{})}
...
class PackageNode(Node): # Same as before
    # ...
    def prep(self, shared: Dict[str, Any]) -> Dict[str, Any]:
        logger.info("Entering PackageNode - Prep"); project_structure=shared.get("generated_project_structure"); planned_desc=shared.get("planned_task_description")
        if not isinstance(project_structure,dict) or not project_structure.get("files"): return {"error":"Generated project structure missing or invalid for packaging."} 
        if not isinstance(planned_desc,dict): return {"error":"Planned task description (object) missing for packaging."}
        return {"generated_project_structure":project_structure,"planned_task_description":planned_desc}
    def exec(self, prep_res: Dict[str, Any]) -> Dict[str, Any]:
        if prep_res.get("error"): return prep_res
        logger.info("PackageNode - Executing"); project_structure=prep_res["generated_project_structure"]; plan=prep_res["planned_task_description"]
        main_component_name=plan.get("component_name") 
        if not main_component_name: logger.error("PackageNode: component_name missing."); return {"error":"component_name missing in planned_task_description for packaging."} 
        packaged_info={"project_files":project_structure.get("files",[]),"entry_point":project_structure.get("entry_point_file","N/A"),"main_component_details":plan}
        handoff_summary=f"Successfully packaged project for component: '{main_component_name}'."; return {"packaged_artifacts_info":packaged_info,"handoff_summary":handoff_summary,"main_component_name":main_component_name}
    def post(self, shared: Dict[str, Any], prep_res: Dict[str, Any], exec_res: Dict[str, Any]):
