...
from utils.database import (
    ...
    log_validation_result, get_validation_log_for_version, # Added get_validation_log_for_version
    log_feedback, get_feedback_history_for_task, 
...
from utils.prompts import ( 
    ...
    CRITIQUE_PROMPT_TEMPLATE, SECURITY_COMPLIANCE_PROMPT_TEMPLATE
)
from flow import ( 
    elicitation_flow, 
    test_design_flow, 
...
LLM_MODELS_CONFIG = {
    ...
    "security_llm":   os.getenv("SECURITY_LLM_MODEL", "gpt-3.5-turbo"), # Model for security node
...
def get_shared_for_flow() -> Dict[str, Any]:
    ...
    keys_to_copy = [
        ...
        "security_compliance_status", "security_compliance_issues", # New
        "user_rejection_reason", "critique_feedback", "feedback_history", "refinement_count",
...
def load_task_into_session(task_id: int):
    ...
    if loaded_db_data:
        logger.info(f"LOAD_TASK: Data found in DB for task {task_id}. DB status: {loaded_db_data.get('ui_stage')}")
        preserve_keys = RAG_CONTEXT_KEYS + ["app_initialized", "rag_contexts_loaded"]
        preserved_values = {k: st.session_state.get(k) for k in preserve_keys}
        st.session_state.clear(); st.session_state.update(preserved_values)
        ...
        if not isinstance(st.session_state.get("feedback_history"), list): st.session_state.feedback_history = []
        if not isinstance(st.session_state.get("validation_issues"), list): st.session_state.validation_issues = []
        if not isinstance(st.session_state.get("security_compliance_issues"), list): st.session_state.security_compliance_issues = [] # New
        logger.info(f"LOAD_TASK: Successfully loaded task {st.session_state.active_task_id}. UI stage set to: {st.session_state.ui_stage}")
        st.session_state.current_error_message = None 
    else:
        st.error(f"LOAD_TASK: Could not load data for task {task_id}.")
        st.session_state.active_task_id = None; st.session_state.ui_stage = "TASK_SELECTION" 
        logger.error(f"LOAD_TASK: Failed to load data for task {task_id}. Staying on TASK_SELECTION.")
...
logger.info(f"Top of app.py: active_task_id = {active_task_id}, ui_stage = {current_ui_stage}")

if current_ui_stage == "TASK_SELECTION":
    st.header("Manage Tasks")
    if st.button("🚀 Start New Project Request", key="start_new_task_btn_v8"):
        reset_for_new_task(go_to_input_stage=True); st.rerun()
    st.subheader("Previous Tasks:"); tasks_summary = get_all_tasks_summary() 
    ...
    if st.button("Start Planning", key=f"submit_init_req_v8_{st.session_state.get('active_task_id', 'new')}"):
        if not user_input.strip(): st.warning("Please describe.")
            ...
            if st.session_state.active_task_id is None: 
                ...
                if not task_id_new: st.error("Failed to create task in DB."); st.stop()
                st.session_state.active_task_id=task_id_new; active_task_id=task_id_new
            else: 
                active_task_id = st.session_state.active_task_id # Use existing
                st.session_state.user_raw_request = user_input; st.session_state.current_request_for_planner = user_input
                update_task_field(active_task_id, "initial_request", user_input)
            logger.info(f"Task {active_task_id}: Starting/Continuing planning for request: {user_input[:50]}...")
            ...
            with st.spinner("Architect & Planner thinking..."):
                try:
                    ...
                    if action=="clarification_needed": next_ui_stage = "CLARIFICATION"
                    ...
                    else: st.session_state.current_error_message=st.session_state.get("current_error_message","Planning fail.")
                    st.session_state.ui_stage=next_ui_stage; update_task_field(active_task_id,"status",next_ui_stage)
...
# ... (INITIAL_CODE_GENERATION stage: ensure update_task_field(active_task_id, "status", st.session_state.ui_stage) on transitions)
...
elif current_ui_stage == "QA_VALIDATE_REVIEW" and active_task_id: # Combined QA, Validation, Security
    st.header(f"2c. Automated QA, Validation & Security Scan (Attempt {st.session_state.refinement_count})")
    ...
    if not shared_for_flow.get("generated_project_structure") or not shared_for_flow.get("generated_test_cases"):
        st.session_state.current_error_message="Missing code/tests for QA & Validation."; 
        st.session_state.ui_stage="HUMAN_REVIEW"; update_task_field(active_task_id,"status","HUMAN_REVIEW"); st.rerun()
    else:
        with st.spinner(f"Running tests, validation & security scan (Attempt {st.session_state.refinement_count})..."):
            try:
                action = qa_validation_security_flow.run(shared_for_flow) # Use the new flow
                ...
                if st.session_state.active_code_version_id:
                    for res in st.session_state.test_results_summary: 
                        log_test_run_result(active_task_id,st.session_state.active_code_version_id,res.get("test_case",{}).get("description","N/A"), res.get("status","error"), res.get("actual_output"), res.get("message","N/A"))
                    if st.session_state.validation_status: 
                        log_validation_result(active_task_id,st.session_state.active_code_version_id,st.session_state.validation_status,st.session_state.validation_issues)
                # Persist security results to tasks table
                update_task_field(active_task_id, "security_status", st.session_state.get("security_compliance_status"))
                update_task_field(active_task_id, "security_issues", st.session_state.get("security_compliance_issues", []))
                
                st.session_state.ui_stage="HUMAN_REVIEW"; update_task_field(active_task_id,"status","HUMAN_REVIEW")
            except Exception as e: 
                logger.error(f"QA/Validation/Security error: {e}",exc_info=True); st.session_state.current_error_message=f"QA/Val/Sec error: {e}"
    ...
    st.subheader("Test Cases & Results:"); # ... (same display logic) ...
    ...
    if sec_status == "pass": st.success("Security & Compliance Checks Passed.")
    elif sec_status in ["fail", "error"]:
        st.error(f"Security & Compliance: {sec_status.capitalize()}")
        if sec_issues: [st.warning(f"- {issue}") for issue in sec_issues]
        else: st.warning("No specific security/compliance issues listed by agent.")
    else: st.info("Security & Compliance check not run or status unknown for this version.")

    overall_pass = st.session_state.all_tests_passed and val_status=="pass" and sec_status == "pass"
    
    c1,c2=st.columns(2)
    if c1.button("✅ Approve Project",disabled=not overall_pass,key=f"approve_v8_{active_task_id}_{st.session_state.refinement_count}"): 
    ...
    if c2.button("❌ Reject & Request Refinement",key=f"reject_v8_{active_task_id}_{st.session_state.refinement_count}"):
        ...
        st.rerun()
    if not overall_pass:st.warning("Cannot approve until all tests, validation, and security checks pass.")
...
#      And ensure they update the task 'status' in DB on UI stage transitions.
    ...
    rejection_reason=st.text_area("Reason for rejection/feedback:",value=st.session_state.user_rejection_reason,height=100,key=f"reject_reason_v8_{active_task_id}")
    if st.button("Submit Feedback & Trigger Refinement",key=f"submit_feedback_v8_{active_task_id}"):
    ...
    with st.spinner("Re-designing tests..."):
        try:
            ...
            if st.session_state.current_error_message or not st.session_state.generated_test_cases:
                st.session_state.current_error_message=st.session_state.get("current_error_message","Test re-design fail."); st.session_state.ui_stage="HUMAN_REVIEW"; update_task_field(active_task_id,"status","HUMAN_REVIEW")
    ...
    if st.button("Start New Project Request",key="start_new_comp_v8_app"): reset_for_new_task(go_to_input_stage=False); st.rerun()
    ...
    if st.button("Start New Request",key="start_new_fail_v8_app"): reset_for_new_task(go_to_input_stage=False);st.rerun()

elif not active_task_id and current_ui_stage != "INPUT_REQUIREMENTS": # Use current_ui_stage
    logger.info(f"No active task ID (current stage: {current_ui_stage}). Ensuring TASK_SELECTION.")
    if current_ui_stage != "TASK_SELECTION": 
        st.session_state.ui_stage = "TASK_SELECTION"
        st.rerun()
